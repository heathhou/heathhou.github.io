...
## 10.6.1 Hashtable 与 HashMap

1. Hashtable: 底层是一个哈希表, 是一个线程安全的集合，单线程集合，速度慢.
...
2. static <T extends Comparable<? super T>> void sort(List<T> list)使用前提：被排序的集合里边存储的元素，必须实现Comparable，重写接口中的方法compareTo定义排序的规则。

3. Comparable接口的排序规则：自己（this） - 参数：升序

4. **Comparable和Comparator**的区别:
